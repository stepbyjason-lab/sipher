...
## 6b. OCR 사다리 — 429 쿨다운/재시도/페이싱 (round-28)

**핵심 실측 교정(2026-07-04)**: Gemini 무료 티어 429 응답의 `quotaId`에
`PerDay`가 포함돼도, 같은 응답에 `RetryInfo.retryDelay`(예: `44s`)가 실려 있으면
**그날 하루 소진이 아니라 일시적** 상태다 — 통제 버스트 실측에서 이 조합이
관측된 뒤 수십 분 안에 소량 호출이 정상 회복됐다. `retryDelay`가 전혀 없는
`PerDay`만 진짜 영구(그날) 소진 후보로 남긴다(`core/llm_free.py::_classify_429`).

이에 따라 `core/ocr_ensemble.py`는 provider를 "dead(영구)" 이분법이 아니라
**쿨다운 → 자동 부활 → 반복 실패 시에만 강등** 3상태로 관리한다:

- **cooling_until**: 429 발생 시 `retry_after`(있으면 그 시간, 없으면 지수
  백오프 `30s→60s→120s`)만큼 쿨다운. 지나면 자동 부활.
- **연속 3회** 쿨다운-재시도 실패가 누적될 때만 이번 세션에서 강등(demoted)한다
  — 1~2회 실패는 강등하지 않는다. 성공 시 연속 실패 카운터는 리셋된다.
- 쿨다운 중인 상위 provider는 **대기하지 않고 skip**하고 곧바로 하위 provider로
  폴백한다(사다리 우선). 유료 Claude 옵트인은 무료 provider가 **전부 demoted**일
  때만 후보가 된다 — 일시 쿨다운 상태만으로는 유료로 넘어가지 않는다.
- 로그 문구는 "영구 소진"·"일마감"을 단정하지 않고 "반복 429로 세션 강등"처럼
  관측 가능한 상태만 표현한다.

**이미지당 1회 재시도**: 후보가 0개이고 cooling 중인 provider가 있으며 그
잔여 쿨다운이 `image_max_wait`(기본 90s, `OCR_IMAGE_MAX_WAIT`) 이내이면, 그
...
모드에서 서로 다른 provider의 연속 호출에는 적용 안 함). `0`으로 설정하면
비활성화된다.
