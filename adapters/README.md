...
| [`threads/`](threads/) | [vdite/threads-scraper](https://github.com/vdite/threads-scraper) fork(vendored, MIT) — [우리 fork](https://github.com/stepbyjason-lab/threads-scraper) | 기본은 fast 원글 작성자 수집 + 미디어 다운로드. 타인 댓글은 `--all-comments`, 조건부 deep은 `--auto`, 전체 reply tree는 `--deep`으로 명시 | 구현+라이브 검증(round-02 Gate5 PASS_WITH_FOLLOWUPS) |
