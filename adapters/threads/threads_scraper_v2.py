...
def parse_post(post_data: Dict) -> Dict:
    ...
    try:
        ...
        images, videos = extract_media(post_data)
        ...
        async def handle_response(response):
            ...
            if any(k in url for k in ["graphql", "api/v1"]):
                try:
                    ...
                    all_posts = iter_thread_posts(json_data)
            ...
            if "/ajax/" in url:
                try:
                    ...
                    all_posts = iter_thread_posts(data)
                    for post_obj in all_posts:
                        parsed = parse_post(post_obj)
                        if parsed and parsed.get("id"):
                            extracted_comments[parsed["id"]] = parsed
...
if __name__ == "__main__":
    ...
    if "--login" in flags:
        asyncio.run(main())
    elif "--logout" in flags:
        asyncio.run(main())
    elif not positional or "--help" in flags:
        print("Usage: python threads_scraper_v2.py <threads_url> [max_pages] [--download]")
        print("       python threads_scraper_v2.py --login")
        print("       python threads_scraper_v2.py --logout")
        sys.exit(1)
    else:
        main_url = positional[0]
        max_pages = int(positional[1]) if len(positional) > 1 else 100
        ...
        if results:
            if do_download:
                count = download_media(results, out_dir="downloads")
                print(f"  Downloaded {count} media file(s) into ./downloads/", file=sys.stderr)
